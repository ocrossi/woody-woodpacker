...
t_woodyData read_store_pheaders(const char *filename) {
    t_woodyData data;
...
void write_output_data(t_woodyData *data) {
    ...
    char *output = malloc(data->file_size + data->payload_size + KEY_SIZE + 1);
    if (output == NULL) {
...
void read_sheaders(t_woodyData *data) {
    ssize_t bytes_read = 0;
    lseek(data->fd, 0, SEEK_SET); // on repart au debut du fichier
...
void infect_output_data(const char *filename, t_woodyData *data) {
    // !! PARSE PHEADERS 
    *data = read_store_pheaders(filename); 
    read_sheaders(data);
...
int main(int argc, char *argv[])
{
    ...
    size_t len = data.file_size - start; 
